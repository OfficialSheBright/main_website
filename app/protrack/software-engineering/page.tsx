...
import { softwareEngineeringConfig } from "@/lib/course-configs/software-engineering";
...
export default function SoftwareEngineeringCourse() {
  const [user, loading] = useAuthState(auth);
  const [enrolled, setEnrolled] = useState(false);
  const [currentTopicId, setCurrentTopicId] = useState("software-basics");
  const [userProgress, setUserProgress] = useState<UserProgress | null>(null);
  const [isMobile, setIsMobile] = useState(false);

  const courseConfig = softwareEngineeringConfig;
